...
def mypitch_eval(estimate_path, GT_csvpath, Method):
    ...
    for i in range(len(estimate_list)):
        ...
        (ref_v, ref_c,
         est_v, est_c) = mir_eval.melody.to_cent_voicing(GT_time,
                                                         ...
                                                         estimate_time,
                                                         estimate_pitch)
        raw_pitch = float(mir_eval.melody.raw_chroma_accuracy(ref_v, ref_c,
                                                              est_v, est_c))
        ...
        pitch_raw_accuray_list.append(raw_pitch)
    ...
    # pitch_raw_accuray_list.sort()
    # pitch_chroma_t50_list.sort()
    # pitch_chroma_t25_list.sort()
    # pitch_chroma_t10_list.sort()
    # middle_raw_accuray = pitch_raw_accuray_list[len(pitch_raw_accuray_list) // 2]
    ...
    # print("raw_chroma_accuracy is: mean: " + str(np.mean(pitch_raw_accuray_list)) + " middle: "+str(middle_raw_accuray) + " bias: +=" +
    #       str(np.std(pitch_raw_accuray_list, ddof=1)))
